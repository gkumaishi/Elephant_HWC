...
conf_change_nicetable <- combined_conf_change %>%
  kable(digits = dec,
        ...
        align = align) %>% 
  kable_styling(full_width = FALSE, position = "center") %>% 
  add_header_above(c(" ", "African Elephant" = 2, "Asian Elephant" = 2)) %>% 
  column_spec(3,border_right = TRUE)

#old colors
#background = c("#ffd3b6", "#a8e6cf")
...
Creating a table to show conflict change among different years
...
africa_conf_change_table <- read_csv(here(HWC_data, "Data/Di Minin HWC", "conflict_change_polygons_byyear_africa.csv")) %>% 
  ...
  arrange(conflict_change) %>% 
  mutate("conflict_change" = c("Strong Decrease", "Moderate Decrease", "No Change", "Moderate Increase", "Strong Increase")) %>% 
  select(conflict_change, ssp1_2030, ssp1_2050, ssp1_2070, ssp3_2030, ssp3_2050, ssp3_2070)
...
combined_sdm_conf_risk_nicetable <- combined_data_nums %>%
  kable(escape = FALSE,
        align = align,
        caption = "<center><strong>Percentage of Range Boundary Within Each Marginal Climatic Suitability Change and Conflict Risk Category for African and Asian Elephants</strong></center>",
        col.names = c("Marginal Suitability Change", "Low", "Medium", "High", "Total", "Low", "Medium", "High", "Total", "Low", "Medium", "High", "Total", "Low", "Medium", "High", "Total")) %>% 
  kable_styling(full_width = FALSE, position = "center") %>% 
...
combined_sdm_conf_change_nicetable <- combined_data_nums %>%
  kable(escape = FALSE,
        ...
        col.names = c("Marginal Suitability Change", 
...
#read in conflict data for masking

conf_africa <- rast(here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/conflict_boundaries/conflict_boundary_rasters_africa/wp2010_current_day_cb_raster.tif"))

conf_asia <- rast(here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/conflict_boundaries/conflict_boundary_rasters_asia/wp2010_current_day_cb_raster_asia.tif"))

#read in baseline SDMs for africa and asia and reproject to match conflict rasters

base_africa_sdm <- rast(here(HWC_data, "/Wallace SDM Rasters/LQ/1981_2010/afel_historic_sdm.tif")) %>% 
  project(conf_africa, method = "near")

base_asia_sdm <- rast(here(HWC_data, "/Wallace SDM Rasters/asia/1981_2010/asian_elephant_historic_raw.tif")) %>% 
  project(conf_asia, method = "near")

#mask SDMs by conflict

base_africa_sdm_masked <- mask(base_africa_sdm, conf_africa)

base_asia_sdm_masked <- mask(base_asia_sdm, conf_asia)

#save cropped rasters for mapping
writeRaster(base_africa_sdm_masked, filename = here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/sdm_exploration_polygons/africa/africa_baseline_sdm_masked.tif"))

writeRaster(base_asia_sdm_masked, filename = here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/sdm_exploration_polygons/asia/asia_baseline_sdm_masked.tif"))

#polygonize for calculations and then save

base_africa_sdm_masked_polygon <- base_africa_sdm_masked %>% 
  terra::as.polygons(trunc=TRUE, dissolve=TRUE, values=TRUE, na.rm=TRUE, na.all=FALSE, extent=FALSE)

writeVector(base_africa_sdm_masked_polygon, filename = here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/sdm_exploration_polygons/africa/africa_baseline_sdm_masked.shp"), overwrite = TRUE)

base_asia_sdm_masked_polygon <- base_asia_sdm_masked %>% 
  terra::as.polygons(trunc=TRUE, dissolve=TRUE, values=TRUE, na.rm=TRUE, na.all=FALSE, extent=FALSE)

writeVector(base_asia_sdm_masked_polygon, filename = here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/sdm_exploration_polygons/asia/asia_baseline_sdm_masked.shp"), overwrite = TRUE)
