...
cb_vect <- terra::as.polygons(conflict_int, trunc=TRUE, dissolve=TRUE, values=TRUE, na.rm=TRUE, extent=FALSE) #removed na.all = FALSE
...
st_write(cb_line, dsn = here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/conflict_boundaries_asia/", name), append = FALSE)
