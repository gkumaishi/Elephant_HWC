...
pol_list <- list.files(path = here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/differenced_polygons_asia"), pattern = '.shp')

#apply the area over the list

area_list <- lapply(pol_list, perc_area_table)

conf_change_table_pol <- cbind(area_list[[2]]$FPOP_SSP1_)
...
write.csv(as.data.frame(conf_change_table_pol), file = here(HWC_data, "Data/Di Minin HWC", "conflict_change_polygons_byssp_asia.csv"), row.names=FALSE)
...
write.csv(conf_change_table_pol_year, file = here(HWC_data, "Data/Di Minin HWC", "conflict_change_polygons_byyear_asia.csv"), row.names=FALSE)

```

```{r}
conf_change_table_pol_year <- read_csv(here(HWC_data, "Data/Di Minin HWC", "conflict_change_polygons_byyear_asia.csv"))
