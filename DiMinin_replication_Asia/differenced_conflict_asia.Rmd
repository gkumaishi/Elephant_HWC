...
polygons <- function(ssp, rcp, year){
  ...
  pol <- rast(here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/differenced_rasters_asia/", file)) %>%
  ...
  writeVector(pol, filename = here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/differenced_polygons_asia/", name), overwrite = TRUE)
...
perc_area_table <- function(x){
  ...
  diff_pol <- read_sf(here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/differenced_polygons_asia", x))
...
pol_list_asia <- list.files(path = here(HWC_data, "/Geospatial Data/Diminin_Replication_Data/differenced_polygons_asia"), pattern = '.shp')

#apply the area over the list

area_list_asia <- lapply(pol_list, perc_area_table)

conf_change_table_pol_asia <- area_list %>% reduce(full_join, by = "conflict_change") %>% 
  units::drop_units()
...
save_kable(conf_change_pols_nicetable, file = here("/Users/mia/Desktop/Fellowships/Arnhold HWC/conf_change_polygons_table_byyear_asia.jpeg"))
```

#Numeric Change

```{r}
s1_inc <- conf_change_table_pol_asia_year$"SSP1 2050"[[4]] + conf_change_table_pol_asia_year$"SSP1 2050"[[5]]

s3_inc <- conf_change_table_pol_asia_year$"SSP3 2050"[[4]] + conf_change_table_pol_asia_year$"SSP3 2050"[[5]]

diff <- s3_inc - s1_inc
diff

s1_dec <- conf_change_table_pol_asia_year$"SSP1 2050"[[1]] + conf_change_table_pol_asia_year$"SSP1 2050"[[2]]

s3_dec <- conf_change_table_pol_asia_year$"SSP3 2050"[[1]] + conf_change_table_pol_asia_year$"SSP3 2050"[[2]]

dec_diff <- s3_dec - s1_dec
dec_diff
```
