...
popdens <- rast(here(HWC_data, "/Geospatial Data/Pop_dens/fpop_data/FPOP_SSP1/FPOP_SSP1_2020.tif")) %>% 
  project(lulc)
...
africa_rast <- rasterize(africa, popdens)
...
basemap_africa <- ne_countries(
  ...
  scale = "medium", 
  returnclass = "sf") %>% 
  st_transform(crs = crs(top10_pop))  # make sure crs is same as raster
