...
population_extract <- extract(population_reproj, assam_HEC_reproj,
                              ...
                              cells = TRUE,
                              na.rm = FALSE) %>% 
  rename(population_n = gpw_v4_population_density_adjusted_to_2015_unwpp_country_totals_rev11_2015_2pt5_min)
```
...
```{r}
ggplot(analysis_table, aes(x = crop_mean)) +
  geom_histogram()

ggplot(analysis_table, aes(x = sdm_max)) +
  geom_histogram()

ggplot(analysis_table, aes(x = water_mean)) +
  geom_histogram()
```
...
```{r}
library(lme4)
reg1 <- glmer(hec ~ sdm_max + crop_mean + (sdm_max * crop_mean) + (1 | year), 
              data = analysis_table, 
