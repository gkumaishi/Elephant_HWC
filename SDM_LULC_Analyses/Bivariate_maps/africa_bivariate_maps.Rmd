...
```{r}
sdm_585_2050[sdm_585_2050 == 1] <- 10
sdm_585_2050[sdm_585_2050 == 2] <- NA
sdm_585_2050[sdm_585_2050 == 3] <- 30
sdm_585_2050[sdm_585_2050 == 4] <- 40
...
```{r}
factor <- ceiling(res(sdm)[1]/res(cropland_126_2050)[1])

cropland_126_2050 <- terra::aggregate(x = cropland_126_2050, fact = factor, fun = mean, na.rm = TRUE)
...
```{r}
crops_126_2050_resampled <- resample(cropland_126_2050, sdm_126_2050, method = "bilinear")
```

#### Make a map of cropland difference and reclassify cropland

We want (future crops - present crops)

0: Decrease in cropland (crops -> no crops)
1: No change
2: Increase in cropland (no crop -> crops)

```{r}
crops_change_585_2050 <- (crops_585_2050_resampled - baseline_afrotrop_crops_resampled)
plot(crops_change_585_2050, main = "Cropland Change")
...
crops_change_585_2050[crops_change_585_2050 >= percentage_difference] <- 2
crops_change_585_2050[crops_change_585_2050 >= -percentage_difference
                      & crops_change_585_2050 < percentage_difference] <- 1
crops_change_585_2050[crops_change_585_2050 < -percentage_difference] <- 0


plot(crops_change_585_2050, main = paste0("2055 SSP5/RCP8.5 Classified Cropland 
                                          Difference from Baseline (+/- ", 100*percentage_difference, " percentage margin)" ), type = "classes", levels = c("Decrease","No Change","Increase"))
```

#### Raster math - Cropland change vs SDM change (SSP5/RCP8.5, 2055)

```{r}
bivariate_change_585_2050 <- as.factor(sdm_585_2050 + crops_change_585_2050)

plot(bivariate_change_585_2050, main = "Cropland change vs SDM change (SSP5/RCP8.5, 2055)", type = "classes", levels = c("-ag -suit", "~ag -suit", "+ag -suit", "-ag ~suit", "~ag ~suit", "+ag ~suit", "-ag +suit", "~ag +suit", "+ag +suit"), col = c("light gray","pink","red","light blue", "lavender", "magenta", "blue", "violet", "purple"))
```
...
```{r}
sdm_126_2080[sdm_126_2080 == 1] <- 10
sdm_126_2080[sdm_126_2080 == 2] <- NA
sdm_126_2080[sdm_126_2080 == 3] <- 30
sdm_126_2080[sdm_126_2080 == 4] <- 40
...
```{r}
factor <- ceiling(res(sdm)[1]/res(cropland_126_2080)[1])

cropland_126_2080 <- terra::aggregate(x = cropland_126_2080, fact = factor, fun = mean, na.rm = TRUE)
...
```{r}
crops_126_2080_resampled <- resample(cropland_126_2080, sdm_126_2080, method = "near")
```
...
```{r}
crops_change_126_2080 <- (crops_126_2080_resampled - baseline_afrotrop_crops_resampled)
plot(crops_change_126_2080, main = "Cropland Change")

# Reclassify cropland change map for easier raster math

# Input percentage difference cutoff for "no change" category
percentage_difference <- 0.01
crops_change_126_2080[crops_change_126_2080 >= percentage_difference] <- 2
crops_change_126_2080[crops_change_126_2080 >= -percentage_difference
                      & crops_change_126_2080 < percentage_difference] <- 1
crops_change_126_2080[crops_change_126_2080 < -percentage_difference] <- 0


plot(crops_change_126_2080, main = paste0("2085 SSP5/RCP8.5 Classified Cropland 
                                          Difference from Baseline (+/- ", 100*percentage_difference, " percentage margin)" ), type = "classes", levels = c("Decrease","No Change","Increase"))
```
...
```{r}
bivariate_change_126_2080 <- as.factor(sdm_126_2080 + crops_change_126_2080)

plot(bivariate_change_126_2080, main = "Cropland change vs SDM change (SSP1/RCP2.6, 2085)", type = "classes", levels = c("-ag -suit", "~ag -suit", "+ag -suit", "-ag ~suit", "~ag ~suit", "+ag ~suit", "-ag +suit", "~ag +suit", "+ag +suit"), col = c("light gray","pink","red","light blue", "lavender", "magenta", "blue", "violet", "purple"))
```
...
```{r}
sdm_585_2080[sdm_585_2080 == 1] <- 10
sdm_585_2080[sdm_585_2080 == 2] <- NA
sdm_585_2080[sdm_585_2080 == 3] <- 30
sdm_585_2080[sdm_585_2080 == 4] <- 40
...
```{r}
crops_126_2080_resampled <- resample(cropland_126_2080, sdm_126_2080, method = "bilinear")
```
...
```{r}
crops_change_585_2080 <- (crops_585_2080_resampled - baseline_afrotrop_crops_resampled)
plot(crops_change_585_2080, main = "Cropland Change")

# Reclassify cropland change map for easier raster math

# Input percentage difference cutoff for "no change" category
percentage_difference <- 0.01
crops_change_585_2080[crops_change_585_2080 >= percentage_difference] <- 2
crops_change_585_2080[crops_change_585_2080 >= -percentage_difference
                      & crops_change_585_2080 < percentage_difference] <- 1
crops_change_585_2080[crops_change_585_2080 < -percentage_difference] <- 0


plot(crops_change_585_2080, main = paste0("2085 SSP5/RCP8.5 Classified Cropland 
                                          Difference from Baseline (+/- ", 100*percentage_difference, " percentage margin)" ), type = "classes", levels = c("Decrease","No Change","Increase"))
```
...
```{r}
bivariate_change_585_2080 <- as.factor(sdm_585_2080 + crops_change_585_2080)
plot(bivariate_change_585_2080, main = "Cropland change vs SDM change (SSP1/RCP2.6, 2085)", type = "classes", levels = c("-ag -suit", "~ag -suit", "+ag -suit", "-ag ~suit", "~ag ~suit", "+ag ~suit", "-ag +suit", "~ag +suit", "+ag +suit"), col = c("light gray","pink","red","light blue", "lavender", "magenta", "blue", "violet", "purple"))
```
